...
var SONG_LIST = ["futura_bodoni", "futura_jenson", "futura_knockout"]
...
$(document).ready(function() {
   ...
   /*---------------------------------*/
	...
	var update_song = function(bool)
	{
		// Going to paired page; fade out loops
		if(bool)
		{
			LOOP_DICT[LEFT_TYPE].fadeOut(1500);
			LOOP_DICT[RIGHT_TYPE].fadeOut(1500);

			var song_name = find_song_name();
			play_song(song_name);

		}
		// Leaving paired page; fade out song
		else
		{
			stop_song();
			LOOP_DICT[LEFT_TYPE].setVolume(LOOP_VOLUME);
			LOOP_DICT[RIGHT_TYPE].setVolume(LOOP_VOLUME);
		}

	}

	var find_song_name = function()
	{
		if ($.inArray(LEFT_TYPE.concat("_").concat(RIGHT_TYPE), SONG_LIST) != -1)
		{
			return LEFT_TYPE.concat("_").concat(RIGHT_TYPE).concat("_song");
		}

		else if ($.inArray(RIGHT_TYPE.concat("_").concat(LEFT_TYPE), SONG_LIST) != -1)
		{
			return RIGHT_TYPE.concat("_").concat(LEFT_TYPE).concat("_song");
		}
		else
		{
			return "";
	...
	var play_song= function(song_name)
	{
		if (song_name == "")
		{
			return;
		...
		else
		{
			...
			{
				SONG_DICT[song_name].play();
			}
			else
			{
				SONG_DICT[song_name] = new buzz.sound(SONGS_FOLDER.concat(song_name), {
					formats: ["mp3"],
					preload: true,
					loop: true,
					autoplay: true,
